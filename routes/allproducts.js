    ...
    if (useSolr === 'true') {
        ...
        try {
            ...
            if (hasCategories) {
                ...
                query += ` AND p.C_id IN (${placeholders})`;
            ...
            db.all(query, params, (err, rows) => {
                ...
                const countQuery = `
                    ...
                    WHERE P_name LIKE ?
                        ...
                        AND no_of_items BETWEEN ? AND ?
                        AND discount_price BETWEEN ? AND ?
                      ${hasCategories ? `AND p.C_id IN (${categoryList.map(() => '?').join(',')})` : ''}
                `;
                const countParams = [
                    `%${search}%`,
                    minPrice,
...
router.get('/categories', async (req, res) => {
    try {
        ...
        db.all('SELECT * FROM categories', (err, rows) => {
            ...
            console.log('Fetched categories:', rows); // Debug log
